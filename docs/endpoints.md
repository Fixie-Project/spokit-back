...
| GET | `/api/posts/` | 누구나 | 게시글 목록. 비스태프는 발행(`published`)만 확인 |
  - `?q=` 파라미터로 제목/본문/브랜드에 키워드 검색 가능 |
  - `?limit=<n>&offset=<n>` 지원 (기본 `limit=9`, 최대 `30`)
  - 응답 `data`: `{count, next, previous, results}` (`results`는 `PostListSerializer` 배열)
  - 응답(`PostListSerializer`): `id`, `author`, `slug`, `main_title`, `thumbnail_image`, `created_at`, `is_editor_pick`, `tags`, `like_count`, `comment_count`(annotate), `is_liked`(로그인 시 사용자 기준, 미로그인 `false`) |
...
- `GET /api/search/?q=<keyword>&type=&sort=&preview_limit=&page=&page_size=` (누구나)
  ...
  - 검색 범위:
    - magazine: `main_title`, `content`, `tags`
